...
namespace EdB.PrepareCarefully
{
	public abstract class Dialog_Colonist : Window
	{
		...
		protected Page_ConfigureStartingPawnsCarefully CharMakerPage
		{
			get {
				return Find.WindowStack.WindowOfType<Page_ConfigureStartingPawnsCarefully>();
			}
		}
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			foreach (FileInfo current in list) {
				...
				if (Widgets.ButtonImage(rect4, Textures.TextureDeleteX)) {
					...
					Find.UIRoot.windows.Add(new Dialog_Confirm("EdB.ConfirmColonistDelete".Translate(new object[] {
						localFile.Name
					}), delegate {
						localFile.Delete();
					}, true, null, true));
				}
				TooltipHandler.TipRegion(rect4, "EdB.DeleteThisColonist".Translate());
				GUI.EndGroup();
				num2 += vector.y + 3;
				num3++;
			}
