...
namespace EdB.PrepareCarefully
{
	...
	public class Page_Equipment : Page_PrepareCarefully
	{
		...
		protected void DrawEquipmentIcon(Rect rect, EquipmentDatabaseEntry entry)
		{
			...
			if (entry.thing == null) {
				GUI.DrawTexture(rect, entry.def.uiIcon);
			}
			else {
				// EdB: Inline copy of static Widgets.ThingIcon() with graphics switched to show a side view instead of front view.
				...
				if (!thing.def.uiIconPath.NullOrEmpty()) {
					...
					if (thing is Pawn) {
						...
						resolvedIcon = pawn.Drawer.renderer.graphics.nakedGraphic.MatSide.mainTexture;
					}
					else {
